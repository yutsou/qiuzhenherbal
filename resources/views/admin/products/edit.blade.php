...
@inject('adminProductEditPresenter', 'App\Presenters\AdminProductEditPresenter')
@section('content')
        ...
        <div class="uk-margin">
            <div class="uk-card uk-card-default uk-card-body">
                ...
                <div class="uk-margin" id="mainImageDisplay">
                    <img uk-img="" src="{{ $product->image_url }}" style="width: 200px; height: 200px;">
                    <input type="text" id="mainImageUrl" name="mainImageUrl" value="{{ $product->image_url }}" hidden="hidden">
                </div>
            ...
            <div class="uk-card uk-card-default uk-card-body">
                ...
                <div class="uk-grid uk-grid-small uk-margin" id="otherImagesDisplay" uk-grid>
                    @foreach ($product->otherImages as $otherImage)
                        <div>
                            <img uk-img="" src="{{ $otherImage->image_url }}" style="width: 200px; height: 200px;">
                            <input type="text" class="otherImageUrls" name="otherImageUrls[]" value="{{ $otherImage->image_url }}" hidden="hidden">
                        </div>
                    @endforeach
                </div>
            ...
            <div class="uk-grid-small uk-child-width-expand@s" uk-grid uk-height-match="target: > div > .uk-card">
                                ...
                                @if(count($rootCategory->children) !== 0 )
                                    <ul class="uk-list">
                                        @foreach($rootCategory->children as $childrenCategory)
                                            {!! $adminProductEditPresenter->presentCheckedCategory($childrenCategory, $product) !!}
                                        @endforeach
                                    </ul>
                                @endif
                            @endforeach
                ...
                <div>
                    <div class="uk-card uk-card-default uk-card-body">
                        ...
                        <ul class="uk-list">
                            @foreach($tags as $tag)
                                {!! $adminProductEditPresenter->presentCheckedTag($tag, $product) !!}
                            @endforeach
                        </ul>
                    </div>
                </div>
        ...
        <div class="uk-margin">
            <div class="uk-card uk-card-default uk-card-body">
                ...
                <div class="uk-form-controls">
                    <input class="uk-input" name="name" type="text" value="{{ $product->name }}">
                </div>
                <h3 class="uk-card-title uk-form-label">商品簡短介紹</h3>
                <div class="uk-form-controls">
                    <input class="uk-input" name="short_description" type="text" value="{{ $product->short_description }}">
                </div>
                <h3 class="uk-card-title uk-form-label">商品提醒</h3>
                <div class="uk-form-controls">
                    <input class="uk-input" name="reminder" type="text" value="{{ $product->reminder }}">
                </div>
                <h3>商品介紹</h3>
                <textarea class="ckeditor" id="introduction">{!! $product->introduction !!}</textarea>
                <h3>商品資訊</h3>
                <textarea class="ckeditor" id="information">{!! $product->information !!}</textarea>
            </div>
        </div>
        <div class="uk-margin">
            <div class="uk-card uk-card-default uk-card-body">
                ...
                <div class="uk-form-controls">
                    <input class="uk-input" name="number" type="text" value="{{ $skus->first()->number }}">
                </div>
            </div>
        </div>
        <div class="uk-margin">
            <div class="uk-grid-small uk-child-width-expand@s" uk-grid uk-height-match="target: > div > .uk-card">
                <div>
                    <div class="uk-card uk-card-default uk-card-body">
                        ...
                        <div class="uk-form-controls">
                            <input class="uk-input" name="regular_price" type="number" min="0" value="{{ intval($skus->first()->regular_price) }}">
                        </div>
                    </div>
                </div>
                <div>
                    <div class="uk-card uk-card-default uk-card-body">
                        ...
                        <div class="uk-child-width-1-2 uk-grid-small uk-margin" uk-grid>
                            <div>
                                <label>
                                    起始時間
                                    <input type="date" class="uk-input" name="discount_start_at" value="{{ $adminProductEditPresenter->timestampToDatetimeLocal($skus->first()->discount_start_at) }}">
                                </label>
                            </div>
                            <div>
                                <label>
                                    結束時間
                                    <input type="date" class="uk-input" name="discount_end_at" value="{{ $adminProductEditPresenter->timestampToDatetimeLocal($skus->first()->discount_end_at) }}">
                                </label>
                            </div>
                        </div>
                        <div id="discountsField">
                            @foreach ($skus->first()->discounts as $i=>$discount)
                                <div class="uk-grid-small discounts uk-grid" id="discount-{{ $i }}" uk-grid="">
                                    <div class="uk-width-1-4@s uk-first-column">
                                        <input class="uk-input mins" id="min-{{ $i }}" name="mins[]" type="number" min="1" value="{{ $discount->min }}" placeholder="最小">
                                    </div>
                                    <div class="uk-width-1-4@s">
                                        <input class="uk-input maxs" id="max-{{ $i }}" name="maxs[]" type="number" min="0" placeholder="最大" value="{{ $discount->max }}"></div>
                                    <div class="uk-width-1-2@s">
                                        <input class="uk-input discountPrices" id="discountPrice-{{ $i }}" name="discount_price[]" type="number" min="0" placeholder="單價" value="{{ intval($discount->discount) }}">
                                    </div>
                                </div>
                            @endforeach
                        </div>
                ...
                <div>
                    <div class="uk-card uk-card-default uk-card-body">
                        ...
                        <select class="uk-select uk-form-controls" name="visible">
                            <option value="1" {{ ( $product->visible === 1) ? 'selected' : '' }}>上架</option>
                            <option value="0" {{ ( $product->visible === 0) ? 'selected' : '' }}>下架</option>
                        </select>
                    </div>
                </div>
                <div>
                    <div class="uk-card uk-card-default uk-card-body">
                        <div class="uk-grid-small uk-child-width-expand@s" uk-grid>
                            ...
                            <div class="uk-text-right">
                                @if ($skus->first()->inventory === null)
                                    <label><input class="uk-checkbox" id="manageInventory" type="checkbox"> 管理庫存</label>
                                @else
                                    <label><input class="uk-checkbox" id="manageInventory" type="checkbox" checked> 管理庫存</label>
                                @endif
                            </div>
                        </div>
                        <div class="uk-margin" id="inventoryFiled">
                            @if ($skus->first()->inventory === null)
                                <select class="uk-select uk-form-controls" name="inventory_status">
                                    <option value="1" {{ ( $skus->first()->inventory_status === 1) ? 'selected' : '' }}>尚有庫存</option>
                                    <option value="2" {{ ( $skus->first()->inventory_status === 2) ? 'selected' : '' }}>補貨中</option>
                                    <option value="0" {{ ( $skus->first()->inventory_status === 0) ? 'selected' : '' }}>售完</option>
                                </select>
                            @else
                                <input type="text" class="uk-input" name="inventory" placeholder="輸入庫存量" value="{{ $skus->first()->inventory }}">
                            @endif
                        </div>
                    </div>
                </div>
    ...
    @if ($product->type === 'general')
        <div class="uk-margin uk-align-right">
            <a class="uk-button custom-button-1" href="{{ route('shop.products.show', $product) }}" target="_blank">查看商品</a>
            <a class="uk-button custom-button-1 product-store" id="generalProductStore">保存</a>
        </div>
    @else
        <div class="uk-card uk-card-default uk-card-body">
            ...
            <form id="group">
                <div id="groupItemsField">
                    <table class="uk-table uk-table-divider">
                        <tbody>
                        @foreach ($product->groupItems as $i=>$groupItem)
                            <tr id="groupItem-{{ $groupItem->sku_id }}"><td>{{ $groupItem->sku->product->name }}</td>
                                <td><input type="text" name="groupSkuIds[]" value="{{ $groupItem->sku_id }}" hidden="hidden"><input type="number" class="uk-input" name="groupItemQuantities[]" min="1" value="{{ $groupItem->quantity }}" placeholder="數量"></td>
                                <td><a class="uk-button uk-button-default uk-width-expand delete-group-item-buttons" skuid="{{ $groupItem->sku_id }}">移除</a></td>
                            </tr>
                        @endforeach
                        </tbody>
                    </table>
        ...
        <div class="uk-margin uk-align-right">
            <a class="uk-button custom-button-1" href="{{ route('shop.products.show', $product) }}" target="_blank">查看商品</a>
            <a class="uk-button custom-button-1 product-store" id="groupProductStore">保存</a>
        </div>
    @endif
@endsection
@push('scripts')
